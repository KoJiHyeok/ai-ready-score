...
1. Confirm `package.json` version is `0.6.0`.
2. Confirm `--ignore` is documented in README.md and AGENTS.md.
3. Confirm `CHANGELOG.md` has a bilingual `v0.6.0` entry.
4. Confirm `docs/release-notes/v0.6.0.md` is ready for the GitHub Release body.
5. Run `npm test`.
6. Run `npm run check`.
7. Run focused `--ignore` smoke tests for JSON, Markdown, Korean, English, CLI patterns, and config `ignore`.
8. Run `npm pack --dry-run` and inspect the packed file list.
9. Commit the release preparation changes.
10. Create the tag with `git tag v0.6.0`.
11. Push the branch with `git push origin main`.
12. Push the tag with `git push origin v0.6.0`.
...
15. Create the GitHub Release from `docs/release-notes/v0.6.0.md`.

### 한국어

1. `package.json` 버전이 `0.6.0`인지 확인합니다.
2. README.md와 AGENTS.md에 `--ignore`가 문서화되어 있는지 확인합니다.
3. `CHANGELOG.md`에 bilingual `v0.6.0` 항목이 있는지 확인합니다.
4. `docs/release-notes/v0.6.0.md`가 GitHub Release 본문으로 준비되었는지 확인합니다.
5. `npm test`를 실행합니다.
6. `npm run check`를 실행합니다.
7. JSON, Markdown, 한국어, 영어, CLI 패턴, config `ignore`에 대한 `--ignore` smoke test를 실행합니다.
8. `npm pack --dry-run`을 실행하고 패키지 포함 파일 목록을 확인합니다.
9. 릴리스 준비 변경 사항을 커밋합니다.
10. `git tag v0.6.0`으로 태그를 생성합니다.
11. `git push origin main`으로 브랜치를 push합니다.
12. `git push origin v0.6.0`으로 태그를 push합니다.
...
15. `docs/release-notes/v0.6.0.md`로 GitHub Release를 생성합니다.
